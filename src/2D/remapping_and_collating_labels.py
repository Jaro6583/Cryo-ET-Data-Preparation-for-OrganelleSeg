        ...
        try:
            with mrcfile.open(file_path, permissive=True) as mrc:
                ...
                if master_mask is None:
                    ...
                    print(f"Initialized master volume with shape: {master_mask.shape}")
                
                # Safety check: ensure all tomograms are the same dimensions
                if data.shape != master_mask.shape:
                    print(f"ERROR: {filename} shape {data.shape} does not match {master_mask.shape}. Skipping.")
                    continue
                ...
                for val in labels_to_map:
                    ...
                    while mapped_id is None:
                        prompt = f"Found label '{val}' in {filename}. What class is this? (Enter ID or name):"
                        user_input = input(prompt).strip().lower()

                        if user_input in lookup:
                            mapped_id = lookup[user_input]
                            print(f"  -> Mapped original label {val} to ID {mapped_id}.")
                        else:
                            print(f"  -> Invalid input. Please enter a valid ID or class name from your JSON.")
...
def main():
    parser = argparse.ArgumentParser(
        description="Collate multiple 3D .mrc/.rec annotation files into a master mapped volume."
    )
    ...
    parser.add_argument(
        ...
        nargs='+',
        required=True,
        help="List of input .mrc or .rec files to collate (separated by spaces)."
    )

    parser.add_argument(
        "-j", "--json",
        default="cryoET_class_mapping.json",
        help="Path to JSON class mapping file (default: cryoET_class_mapping.json)."
    )

    parser.add_argument(
        "-o", "--output",
        default="master_compiled_labels.mrc",
        help="Output filename for the compiled .mrc file (default: master_compiled_labels.mrc)."
    )
